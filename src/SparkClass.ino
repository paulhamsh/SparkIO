...
void SparkClass::send_bt()
{
   int i;

   // if multiple blocks then send all but the last - the for loop should only run if last_block > 0
   for (i = 0; i < last_block; i++) {
      sp.bt.write(buf[i], BLK_SIZE);
 
   }
      
   // and send the last one      
   sp.bt.write(buf[last_block], last_pos+1);

}

void SparkClass::send_receive_bt()
{
   int i;
   int rec;
...
int SparkClass::get_block(int block)
{
   ...
   for (i=7; i<len; i++) {
      ...
      if (val < 0) return val;
         ...
         while (sp.bt.available()) sp.bt.read();
...
byte SparkClass::read_byte()
{
   ...
   if (multi) {
      ...
      slice_pos = 8 * int (temp_pos / 7);
      byte_pos = temp_pos % 7;
      bit_pos = chunk_offset + 39*chunk + slice_pos + 6;
      pos = chunk_offset + 39*chunk + byte_pos + slice_pos + 7;
   }
   else {
      temp_pos = data_pos;
      slice_pos = 8 * int (temp_pos / 7);
      byte_pos = temp_pos % 7;
      bit_pos = chunk_offset + slice_pos + 6;
      pos = chunk_offset + byte_pos + slice_pos + 7;
   }
